...
-- Insert the 5 predefined housing types
INSERT INTO public.housing_types (name, description) VALUES
...
CREATE TABLE public.dhq_living_units (
  ...
  housing_type_id UUID REFERENCES public.housing_types(id) NOT NULL,
...
INSERT INTO public.dhq_living_units (
  quarter_name, location, category, housing_type_id, no_of_rooms, status, 
  type_of_occupancy, bq, no_of_rooms_in_bq, block_name, flat_house_room_name
) VALUES
('Officers Quarter A', 'Main Base', 'Officer', (SELECT id FROM housing_types WHERE name = 'Three Bedroom Flat'), 3, 'Occupied', 'Single', true, 2, 'Block A', 'Flat 101'),
('Soldiers Quarter B', 'East Wing', 'Men', (SELECT id FROM housing_types WHERE name = 'One Bedroom Flat'), 1, 'Vacant', 'Single', false, 0, 'Block B', 'Room 201'),
('Officers Quarter C', 'West Wing', 'Officer', (SELECT id FROM housing_types WHERE name = 'Duplex'), 4, 'Occupied', 'Single', true, 1, 'Block C', 'House 301'),
('Navy Quarter D', 'Naval Base', 'Officer', (SELECT id FROM housing_types WHERE name = 'Two Bedroom Flat'), 2, 'Vacant', 'Shared', false, 0, 'Block D', 'Flat 401'),
('Air Force Quarter E', 'Air Base', 'Officer', (SELECT id FROM housing_types WHERE name = 'One Bedroom Self Contained'), 1, 'Occupied', 'Single', false, 0, 'Block E', 'Unit 501');
