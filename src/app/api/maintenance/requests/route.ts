...
export async function GET() {
  try {
    ...
    const transformedRequests = requests.map(request => ({
      ...
      quarterName: request.unit?.quarterName || '',
      location: request.unit?.location || '',
      blockName: request.unit?.blockName || '',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const transformedRequest = {
      ...
      quarterName: newRequest.unit?.quarterName || '',
      location: newRequest.unit?.location || '',
      blockName: newRequest.unit?.blockName || '',
