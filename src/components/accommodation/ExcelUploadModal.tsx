...
export function ExcelUploadModal({
  ...
  const validCategories = ["Officer", "NCOs"];
    ...
    rows.forEach((row, index) => {
      ...
      if (row.category && !validCategories.includes(row.category)) {
        validationErrors.push({
          row: rowNum,
          field: "category",
          message: `Category must be either Officer or NCOs`,
        });
      }

      if (row.maritalStatus && !validMaritalStatuses.includes(row.maritalStatus)) {
        validationErrors.push({
          row: rowNum,
          field: "maritalStatus",
      ...
      for (let i = 1; i <= 6; i++) {
        ...
        if (row[nameField]) {
          ...
          } else if (isNaN(Number(row[ageField])) || Number(row[ageField]) < 0 || Number(row[ageField]) > 120) {
            validationErrors.push({
              row: rowNum,
              field: String(ageField),
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel file containing queue personnel data and their accommodation unit assignments
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="border rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <p className="text-gray-600">NCOs:</p>
                  <p className="font-medium">
                    {data.filter(d => d.category === "NCOs").length}
                  </p>
                </div>
                <div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-amber-700 space-y-1">
                <li>• All listed units will be marked as &quot;Occupied&quot;</li>
