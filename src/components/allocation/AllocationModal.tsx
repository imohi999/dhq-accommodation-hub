...
import { Home, Users, MapPin, Building, Phone, Calendar, User, Shield } from "lucide-react";
...
export const AllocationModal = ({ isOpen, onClose, personnel }: AllocationModalProps) => {
  ...
  const filteredUnits = eligibleUnits.filter(unit =>
  ...
  const handleAllocate = async () => {
    ...
    console.log("Selected unit:", selectedUnit.quarter_name);
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select an available {personnel.category} unit for {personnel.full_name}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-3">
                  <h4 className="font-semibold text-sm text-muted-foreground border-b pb-1">BASIC INFORMATION</h4>
                ...
                {/* Service Information */}
                <div className="space-y-3">
                  <h4 className="font-semibold text-sm text-muted-foreground border-b pb-1">SERVICE INFORMATION</h4>
                  <div className="space-y-2">
                    <div>
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground">Arm of Service</p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{personnel.appointment || "N/A"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <h4 className="font-semibold text-sm text-muted-foreground border-b pb-1">PERSONAL & FAMILY</h4>
                  <div className="space-y-2">
                    <div>
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground">Adult Dependents</p>
                      <p className="font-medium">{personnel.no_of_adult_dependents}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Child Dependents</p>
                      <p className="font-medium">{personnel.no_of_child_dependents}</p>
                    </div>
                    <div>
                ...
                <div className="space-y-3 md:col-span-2 lg:col-span-3">
                  <h4 className="font-semibold text-sm text-muted-foreground border-b pb-1">SERVICE DATES</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground">Date TOS</p>
                      <p className="font-medium">{formatDate(personnel.date_tos)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Date SOS</p>
                      <p className="font-medium">{formatDate(personnel.date_sos)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Entry Date</p>
                      <p className="font-medium">{formatDate(personnel.entry_date_time)}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Label htmlFor="search">Search Available {personnel.category} Units</Label>
            ...
            {eligibleUnits.length === 0 ? (
              <div className="text-center p-8 text-muted-foreground">
                ...
                <p className="text-sm">Only units matching the personnel's category ({personnel.category}) are shown</p>
              </div>
            ) : (
              <div>
                  ...
                  <SelectContent>
                    {filteredUnits.map((unit) => (
                      <SelectItem key={unit.id} value={unit.id}>
                        ...
                        ({unit.no_of_rooms} rooms) - {unit.location}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={!selectedUnitId || allocationLoading || unitsLoading || eligibleUnits.length === 0}
          >
